...
func NewHeaderMatcher(header *envoy_api_v2_route.HeaderMatcher) (HeaderMatcher, error) {
	switch header.HeaderMatchSpecifier.(type) {
	case *envoy_api_v2_route.HeaderMatcher_ExactMatch:
		return &ExactStringMatcher{
			ExactMatch: header.HeaderMatchSpecifier.(*envoy_api_v2_route.HeaderMatcher_ExactMatch).ExactMatch,
		}, nil
	case *envoy_api_v2_route.HeaderMatcher_PrefixMatch:
		return &PrefixStringMatcher{
			PrefixMatch: header.HeaderMatchSpecifier.(*envoy_api_v2_route.HeaderMatcher_PrefixMatch).PrefixMatch,
		}, nil
	case *envoy_api_v2_route.HeaderMatcher_SuffixMatch:
		return &SuffixStringMatcher{
			SuffixMatch: header.HeaderMatchSpecifier.(*envoy_api_v2_route.HeaderMatcher_SuffixMatch).SuffixMatch,
		}, nil
	case *envoy_api_v2_route.HeaderMatcher_RegexMatch:
		if rePattern, err := regexp.Compile(
			header.HeaderMatchSpecifier.(*envoy_api_v2_route.HeaderMatcher_RegexMatch).RegexMatch); err != nil {
	...
	case *envoy_api_v2_route.HeaderMatcher_PresentMatch:
		return &HeaderMatcherPresentMatch{
			PresentMatch: header.HeaderMatchSpecifier.(*envoy_api_v2_route.HeaderMatcher_PresentMatch).PresentMatch,
		}, nil
	case *envoy_api_v2_route.HeaderMatcher_RangeMatch:
		return &HeaderMatcherRangeMatch{
			Start: header.HeaderMatchSpecifier.(*envoy_api_v2_route.HeaderMatcher_RangeMatch).RangeMatch.Start,
			End:   header.HeaderMatchSpecifier.(*envoy_api_v2_route.HeaderMatcher_RangeMatch).RangeMatch.End,
		}, nil
	default:
